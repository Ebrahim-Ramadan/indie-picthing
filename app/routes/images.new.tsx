...
type ActionData =
  ...
  export const action: ActionFunction = async ({ request }) => {
    ...
    try {
        ...
        for (const image of images) {
          if (image instanceof File) {
            const createdImage = await createImage({
              originalUrl: `/uploads/${image.name}`,
              userId,
            });
            ...
            const bgRemovedUrl = `/uploads/bg-removed-${image.name}`;
      ...
      return json<ActionData>(
        { errors: { image: "File is required and must not be empty" } },
        { status: 400 }
      );
    } catch (error) {
  ...
  export default function NewImagePage() {
    ...
    const handleFiles = (files: FileList) => {
      ...
      formData.append("intent", "upload"); // Ensure intent is set if required
